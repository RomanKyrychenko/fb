...
remDr <- remoteDriver(remoteServerAddr = "localhost", port = 4448L, browserName = "firefox")
remDr$open(silent = T)
remDr$navigate("http://www.facebook.com")

fb_login(id = "+380633960623", pass = "8938006230")

for (i in 1:50000) {
  tryCatch({
    if(check_sql(con)) con <- connect_sql()
    jj <- exezekutor(persons[576:767])$result
    jj <- jj[!is.na(jj)]
